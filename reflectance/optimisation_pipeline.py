...
class OptPipe():
    ...
    def fit_spectra(self):
        ...
        self.fitted_params = pd.DataFrame(
            fitted_params, index=self.spectra.index, columns=['bb', 'K', 'H'] + list(list(self.endmembers.index)))
    ...
    def generate_spectra_from_fits(self):
        self.fitted_spectra = spectrum_utils.generate_spectra_from_fits(
            self.fitted_params, self.spectra.columns, self.endmembers.values, self.aop_args)
    ...
    def generate_results_summary(self):
        ...
        self.results_summary = pd.concat(
            [self.metadata_df, self.glob_summary, self.cfg_df, metrics_summary, self.error_df], axis=1)
    ...
    def generate_error_summary(self):
        ...
        error_df = pd.DataFrame(
            [[self.run_id, self.e_step, error_message]], columns=['run_id', 'step', 'error_message'])
